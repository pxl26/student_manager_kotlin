    ...
    suspend fun showRequestedSummary2(selectedSemester: String) : ArrayList<Summary> {
        ...
        for (item in classList) {
            ...
            if (summary.numOfStudents == 0) {
                ...
                val studentList = ClassFirebase.getStudentInClass(aClass.name)
                for (student in studentList) {
                    // Kiểm tra nếu học sinh vượt qua tất cả các môn
                    var isPassedAll = true
                    for (subject in Constraint.NamesOfSubject) {
                        val transcript = student.transcript?.get("${subject}/${selectedSemester}")
                        if (transcript == null || (transcript.grade15 + transcript.grade45 + transcript.semesterGrade) / 3 < Constraint.AdmissionScore)  {
                            isPassedAll = false
                            break
                        }
                    }
